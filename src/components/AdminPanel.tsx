...
export default function AdminPanel({
  ...
  // Filter lists
  const approvedWorkers = safeUsers.filter(u => u.role === 'WORKER' && u.status === 'APROBADO');
  const pendingRequests = safeUsers.filter(u => u.status === 'PENDIENTE_APROBACION');
  const tasksPendingReview = safeTasks.filter(t => t.status === 'PENDIENTE_REVISION');

  // Metrics calculations
  const totalTasks = safeTasks.length;
  const completedTasksCount = safeTasks.filter(t => t.status === 'APROBADA').length;
  const pendingTasksCount = safeTasks.filter(t => t.status === 'PENDIENTE' || t.status === 'ACEPTADA' || t.status === 'EN_PROCESO').length;
  ...
  const statusChartData = [
    { name: 'Pendientes', valor: safeTasks.filter(t => t.status === 'PENDIENTE').length },
    { name: 'Aceptadas', valor: safeTasks.filter(t => t.status === 'ACEPTADA').length },
    { name: 'En Proceso', valor: safeTasks.filter(t => t.status === 'EN_PROCESO').length },
    { name: 'Revisiones', valor: safeTasks.filter(t => t.status === 'PENDIENTE_REVISION').length },
    { name: 'Aprobadas', valor: safeTasks.filter(t => t.status === 'APROBADA').length },
    { name: 'Rechazadas', valor: safeTasks.filter(t => t.status === 'RECHAZADA').length },
  ];
                  ...
                  {tasksPendingReview.length === 0 ? (
                    ...
                    tasksPendingReview.map((task, index) => (
                      <div key={task.id || `review-${index}`} className="bg-white dark:bg-gray-900 border border-gray-150 dark:border-gray-800 p-5 rounded-xl space-y-4 shadow-sm">
                        ...
                        {(() => {
                          const lastItem = Array.isArray(task.history) && task.history.length > 0 ? task.history[task.history.length - 1] : null;
                          if (!lastItem || !lastItem.comment) return null;
                          return (
              ...
              >
                ...
                <div className="bg-white dark:bg-gray-900 border border-gray-150 dark:border-gray-800 rounded-xl overflow-hidden shadow-sm divide-y divide-gray-100 dark:divide-gray-800">
                  {safeLogs.length === 0 ? (
                    ...
                    safeLogs.slice(0, 50).map((log, index) => (
                      <div key={log.id || `log-${index}`} className="p-3.5 flex items-start justify-between gap-4 text-xs hover:bg-gray-50 dark:hover:bg-gray-800/20">
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="font-mono font-black text-gray-800 dark:text-gray-300 uppercase px-1.5 bg-gray-100 dark:bg-gray-800 rounded text-[10px]">{(log.action || '').replace(/_/g, ' ')}</span>
